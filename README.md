...
3. 选择要分成的页数，再为每一页分别选择排版方式和模板。
4. 根据需要调整画布大小、间距、边距、圆角和背景色。
5. 在右侧切换页面，检查并调整图片位置。
6. 可以导出当前页、多选页面或全部页面；PNG 文件会直接下载到浏览器的下载位置。
...
- 长按图片后拖动到其他页面标签，再点击目标图片：跨页交换两张图片。
- 悬浮在图片上点击右上角 `x`：删除该图片。
- 悬浮在图片上滚轮：缩放该图片的裁剪范围。

## 多页拼图

- 在“分成页数”中输入页数，然后点击“重新分页”，图片会尽量平均分布到各页。
- 每一页可以独立选择自动网格、等格网格或模板，切换页面不会覆盖其他页面的排版。
- 右侧页面标签用于切换预览；“导出”菜单支持导出所选页面或全部页面。
- 左侧“导出当前页”只导出正在预览的页面。
